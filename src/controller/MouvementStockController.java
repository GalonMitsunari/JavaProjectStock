...
public class MouvementStockController {
	...
	private void loadMouvements() {
		try {
			...
			for (MouvementStock mouvement : mouvementStockDAO.getAllMouvements()) {
				mouvementListView.getItems().add("Produit ID: " + mouvement.getIdProduit()
						+ ", Type: " + mouvement.getTypeMouvement() + ", Quantité: "
						+ mouvement.getQuantite() + ", Rack ID: "
						+ (mouvement.getIdRack() != null ? mouvement.getIdRack()
								: "N/A")
	...
	private void addMouvement() {
		try {
			...
			Integer idRack = rackIdField.getText().isEmpty() ? null
					: Integer.parseInt(rackIdField.getText());

			MouvementStock mouvement = new MouvementStock(0, idProduit, typeMouvement, quantite,
					idRack, LocalDateTime.now());
